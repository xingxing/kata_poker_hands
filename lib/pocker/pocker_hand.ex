defmodule Poker.PokerHands do
  ...
  def main(game) do
    try do
      [[_, player1, black_cards, player2, white_cards]] = Regex.scan(~r/(\S+): (.{14}) (\S+): (.{14})/, game)
      judge(
        {player1, black_cards |> sort |> compute_cards_rank},
        {player2, white_cards |> sort |> compute_cards_rank})
    rescue
      _ ->
        raise("Input Error!")
    end
